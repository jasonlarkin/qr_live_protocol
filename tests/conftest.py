...
def mock_time_provider():
    ...
    with patch('src.time_provider.TimeProvider') as mock:
        ...
        provider.get_time_server_verification.return_value = {
            "pool.ntp.org": "0.001"
        }
...
@pytest.fixture
def qrlp_instance(sample_config, mock_time_provider, mock_blockchain_verifier, 
                  mock_identity_manager, mock_qr_generator) -> QRLiveProtocol:
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "web: mark test as requiring web interface"
    )
